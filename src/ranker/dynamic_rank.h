...
//           phraseTerms should be ordered like they are in the query
//           Phrase ISR's are used for the number of phrase matches.
//           Needs a start location so we can seek to that location
// Modifies: Nothing.
// Effect: Returns the dynamic rank score for a single document.
int get_dynamic_rank(ISRWord* anchorTerm, vector<ISRWord*> phraseTerms, ISRPhrase* phrase, ISREndDoc* endDoc, uint64_t startLocation, cstring_view type) {
    ...
    while (anchorTerm->GetCurrentPost()->location < endLoc) {
        ...
        for (int i = 0; i < phraseTerms.size(); i++) {
            auto currPost = phraseTerms[i]->GetCurrentPost();
            // if its not the anchor term and is still in the document do the math
            if (phraseTerms[i] != anchorTerm && currPost->location < endLoc) {
