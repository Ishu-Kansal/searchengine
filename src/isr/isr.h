...
class ISR {
 public:
  ...
  virtual Location getStartLocation() { return nearestStartLocation; }
  virtual Location getEndLocation() { return nearestEndLocation; }

  virtual SeekObj* GetCurrentPost() const { return currPost.get(); }
...
class ISRWord : public ISR {
 public:
  ...
  SeekObj* NextDocument() override { return Seek(getEndLocation() + 1); }
  ...
  Location getStartLocation() override { return nearestStartLocation; }
  Location getEndLocation() override { return nearestEndLocation; }
  /*
...
class ISREndDoc : public ISRWord {
 public:
  ...
  ISREndDoc(const IndexFileReader& reader) : ISRWord(endDoc, reader) {}
...
class ISROr : public ISR {
 public:
  ...
  ISROr(std::vector<std::unique_ptr<ISR>> childISRs,
    ...
    for (size_t i = 0; i < terms.size(); ++i) {
      ...
      if (seekObj->location < terms[nearestTerm]->GetCurrentPost()->location) {
        ...
        nearestStartLocation =
            docEnd->location - docEndISR->GetDocumentLength();
 ...
 private:
  ...
  // nearStartLocation and nearestEndLocation are the start and end of the
  // nearestTerm.
...
class ISRAnd : public ISR {
 public:
  std::vector<std::unique_ptr<ISR>> terms;
  std::unique_ptr<ISREndDoc> docEndISR;
  ISRAnd(std::vector<std::unique_ptr<ISR>> childISRs,
         const IndexFileReader& reader)
      : terms(std::move(childISRs)),
        nearestTerm(0),
        farthestTerm(0),
  ...
  SeekObj* Seek(Location target) override {
    ...
    for (size_t i = 0; i < terms.size(); ++i) {
      // 1. Seek all the ISRs to the first occurrence beginning at the target
      // location.
      SeekObj* seekObj = terms[i]->Seek(target);
    ...
    while (true) {
      // 2. Move the document end ISR to just past the furthest word, then
      // calculate the document begin location.
      anotherOne = false;
      ...
      for (size_t i = 0; i < terms.size(); ++i) {
        ...
        if (seekObj->location <
            terms[nearestTerm]->GetCurrentPost()->location) {
          nearestTerm = i;
        }
      }
      if (!anotherOne) break;
...
class ISRPhrase : public ISR {
 public:
  ...
  ISRPhrase(std::vector<std::unique_ptr<ISR>> childISRs,
            const IndexFileReader& reader)
      : terms(std::move(childISRs)),
        farthestTerm(0),
        farthestTermLocation(0),
        nearestStartLocation(0),
        nearestEndLocation(0) {
    docEndISR = std::make_unique<ISREndDoc>(reader);
  }
  SeekObj* Seek(Location target) override {
    ...
    // 1. Seek all ISRs to the first occurrence beginning at the target
    // location.
    for (size_t i = 0; i < terms.size(); ++i) {
      SeekObj* seekObj = terms[i]->Seek(target);
      if (!seekObj) return nullptr;
    ...
    // first location beginning where they should appear relative to the
    // furthest term.
    while (true) {
      anotherOne = false;
      for (size_t i = 0; i < terms.size(); ++i) {
...
class ISRContainer : public ISR {
 public:
  ...
  ISRContainer(std::vector<std::unique_ptr<ISR>> childISRs,
               std::vector<std::unique_ptr<ISR>> excludedISRs,
               const IndexFileReader& reader)
      : terms(std::move(childISRs)),
        excluded(std::move(excludedISRs)),
        nearestTerm(0),
        farthestTerm(0),
        nearestStartLocation(0),
        nearestEndLocation(0) {
  ...
  SeekObj* Seek(Location target) override {
    // 1. Seek all the included ISRs to the first occurrence beginning at the
    // target location.
    if (terms.empty()) return nullptr;
    while (true) {
      ...
      // 2. Move the document end ISR to just past the furthest contained ISR,
      // then calculate the document begin location.
      SeekObj* docEnd =
          docEndISR->Seek(terms[farthestTerm]->GetCurrentPost()->location + 1);
      if (!docEnd) return nullptr;
      nearestStartLocation = docEnd->location - docEndISR->GetDocumentLength();
      nearestEndLocation = docEnd->location;
      ...
      // 5. If any ISR reaches the end, there is no match.
      // 6. Seek all the excluded ISRs to the first occurrence beginning at the
      // document begin location.
      // 7. If any excluded ISR falls within the document, reset the target to
      // one past the end of the document and return to step 1.
      while (true) {
        // 3. Seek all the other terms to past the document begin.
        anotherOne = false;
        for (size_t i = 0; i < terms.size(); ++i) {
