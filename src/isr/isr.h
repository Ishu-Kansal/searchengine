...
class ISROr : public ISR {
 public:
      ...
      if (currPost != nullptr) {
        ...
        if (currStart < nearestStartLocation) {
          nearestStartLocation =
              currStart;  // Update the nearest start location
  ...
  Post* Next() {
    ...
    for (unsigned i = 0; i < NumberOfTerms; i++) {
      Post* currPost =
          Terms[i]->getCurrentPost();  // Get the current post for each term
      if (currPost != nullptr) {
        Location currStart = Terms[i]->GetStartLocation();
        if (currStart < nearestStartLocation) {
          nearestStartLocation =
...
class ISRAnd : public ISR {
 public:
  ...
  Post* Next() { return Seek(nearestStartLocation + 1); }
...
class ISRContainer : public ISR {
 public:
  ...
  Post* Seek(Location target) {
    ...
    // 7. If any excluded ISR falls within the document, reset the target to one
    // past the end of the document and return to step 1.
  }
  Post* Next() { Seek(Contained[nearestContained]->GetStartlocation() + 1); }
