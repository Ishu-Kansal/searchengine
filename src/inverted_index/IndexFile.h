...
class IndexFile {
 ...
 public:
  IndexFile(const char* filename, IndexChunk& indexChunk) {
    ...
    Don't need to reconstruct hash table: hash table has offset of word in
    vector of posting lists (listOfpostingList), use another vector
    (postingListOffsetVector) to store byte offset. Simplest way with our
    current structure.
    ...
    */
    ...
    // TODO: used vector so that we didn't need to calculate size of buffer
    // beforehand Every insert is at the end of the dataBuffer, so should be
    // amortized constant time (constant since we reserved 1 gb)
    std::vector<uint8_t> dataBuffer;
    ...
    ssize_t bytesWritten =
        write(fileDescrip, dataBuffer.data(), dataBuffer.size());
    if (bytesWritten == -1) {
