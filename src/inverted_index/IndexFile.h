 ...
 */
constexpr size_t BLOCK_OFFSET_BITS = 6;
/**
...
// Didn't use pushVarint for some of them, because some of the values are one
// byte and I want the full range from 0 - 255.
 ...
 * @param outFilename Buffer to store the generated filename (for error
 * messages)
...
/**
 * @class IndexFile
 * @brief Handles the serialization and writing of an IndexChunk and its
 * dictionary to disk
 *
 * Class takes an IndexChunk, serializes its URL list and posting lists,
 * updates the dictionary with the byte offsets of the posting lists within the
 * serialized data, writes the serialized chunk data to a file named
 * "IndexChunk_XXXXX", and writes the dictionary to hash table file named
 * "HashFile_XXXXX"
 */
class IndexFile {
 private:
  /**
   * @brief File descriptor for the main postingIndex chunk data file
   * ("IndexChunk_XXXXX").
   */
  FileDescriptor fileDescrip_;

  /**
   * @brief Encodes a uint64_t value using variable-length encoding (Varint) and
  ...
  /**
   * @brief Serializes the list of URLs (Doc objects) into the provided data
   * buffer
   *
   * Format:
   * [Seek Table Size (Varint Size Byte)]  // 1 byte for size of next field, 0
   * if no seek table [Number of Seek Table Entries (Varint)] // Only if Seek
   * Table Size > 0 [Seek Table Entries...]             // Only if Seek Table
   * Size > 0 [Offset (uint64_t)]               // Byte byteOffset of the start
   * of the block [URL Data...] [URL Length (uint8_t)]            // Length of
   * the URL string [URL String (bytes)]              // URL characters [Static
   * Rank (uint8_t)]           // Static rank of the document
   *
   * A seek table is only included if the number of URLs is >= BLOCK_SIZE.
   * Each seek table entry points to the start of a block containing BLOCK_SIZE
   * URLs
   *
   ...
   * 1. Updates the corresponding word's entry in the dictionary with the
   * current byte byteOffset in the data buffer. This byteOffset marks the start
   * of the serialized data for this posting list.
   * 2. Serializes the PostingList data:
   *    Format:
   *    [Seek Table Size (Varint Size Byte)]    // 1 byte for size of next
   * field, 0 if no seek table [Number of Seek Table Entries (Varint)] // Only
   * if Seek Table Size > 0 [Number of Postings (Varint)]           // Total
   * number of postings in the list [Seek Table Entries...]                 //
   * Only if Seek Table Size > 0 [Offset (uint64_t)]                     // Byte
   * byteOffset within the posting data (after headers/seek table) [Absolute
   * Location (uint64_t)] [Posting Data...] [Delta (Varint)] // Difference from
   * the prevLocationious post's location
   *
   * A seek table is included if the number of postings is >= BLOCK_SIZE
   * Each seek table entry points to the start of a block containing BLOCK_SIZE
   * postings
   *
   * @param dataBuffer The vector of bytes to append the serialized posting
   * lists to
   * @param listOfPostingList The vector of PostingList objects to serialize
   * @param dictionary The HashTable mapping words to offsets, which will be
   * updated
   */

  void serializePostingLists(std::vector<uint8_t>& dataBuffer,
                             const std::vector<PostingList>& listOfPostingList,
    ...
    for (const PostingList& postingList : listOfPostingList) {
      ...
      // dataBuffer.size() is the byte byteOffset for the start of this posting
      // list
   ...
   * @param dictionary The HashTable associated with the chunk to be updated
   * with offsets
   */
  void serializeChunk(const IndexChunk& indexChunk,
 ...
 public:
  /**
   * @brief Constructs an IndexFile, serializing and writing an IndexChunk and
   * its dictionary
   *
   * Takes a chunk number and an IndexChunk reference. It performs the following
   * steps:
   * 1. Creates or truncates a file named "IndexChunk_XXXXX" (where XXXXX is the
   * zero-padded chunk number)
   * 2. Creates a large buffer to hold the serialized data
   * 3. Calls serializeChunk to fill the buffer with the serialized URL list and
   * posting lists During serialization, the dictionary within indexChunk is
   * updated with the byte offsets of each posting list within the buffer
   * 4. Writes the contents of buffer to the "IndexChunk_XXXXX" file
   * 5. Creates a HashFile named "HashFile_XXXXX", passing the updated
   * dictionary to it This HashFile constructor handles writing the dictionary
   * to disk
   *
   * @param chunkNum The identifier for this chunk, used in filenames. Max 99999
   * @param indexChunk A reference to the IndexChunk object containing the data
   * to be serialized and written
   *
   */
  IndexFile(uint32_t chunkNum, IndexChunk& indexChunk)
      : fileDescrip_(openIndexChunkFile(chunkNum, indexFilename_,
                                        sizeof(indexFilename_))) {
    HashTable<const std::string, size_t>& dictionary =
        indexChunk.get_dictionary();
    ...
    std::vector<uint8_t> dataBuffer;
    dataBuffer.reserve(
        CHUNK_BUFFER_INITIAL_RESERVE);  // 8 Gigabyte worth of bytes
