...
Unicode GetUtf8(const Utf8 *p) {
    ...
    switch (length) {
        case 1:
            if (p[0] >> 7) return ReplacementCharacter;
            return p[0];
        case 2:
            if ((p[0] & 0b00011110) == 0b0000) return ReplacementCharacter;
            if ((p[1] >> 6) != 0b10) return ReplacementCharacter;
...
const Utf8 *NextUtf8(const Utf8 *p, const Utf8 *bound) {
    ...
    const Utf8 *expected = p + initLength;
    switch (initLength) {
        case 1:
            return p + 1;
        case 2:
            if (!(p[0] & 0b00111110)) {
                std::cerr << "case 2";
                return p + 1;
            } else if (bound && p + 2 > bound)
                return bound;
            return p + 2;
        case 3:
            if (((p[0] >> 3) == 0b111) && ((p[1] >> 5) & 0b001)) {
                std::cerr << "case 3";
                return p + 2;
            } else if (bound && p + 3 > bound)
                return bound;
            return p + 3;
        case 4:
            if ((p[0] & 0b00001111) && ((p[1] >> 4) & 0b0111))
                return p + 3;
            else if (bound && p + 4 > bound)
                return bound;
            return p + 4;
        case 5:
            if ((p[0] & 0b00000111) && ((p[1] >> 3) & 0b01111))
                return p + 4;
            else if (bound && p + 5 > bound)
                return bound;
            return p + 5;
        default:
            if (p + 6 > bound) return bound;
            return p + 6;
    }
}
...
Utf8 ToLower(const Utf8 *a) {
    if (*a >= 0b01000001 && *a <= 0b01011010) return *a + 26;
}
...
Unicode ToLower(Unicode c) {
    if (c >= 0b01000001 && c <= 0b01011010) return c + 26;
}
